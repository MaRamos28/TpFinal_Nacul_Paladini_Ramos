...
def render_texto(texto:str, tamaño:int, color:tuple, ruta_fuente_pvz:str="Letra/ZOMBIE.TTF") -> pygame.Surface:
    ...
    try:
        if not isinstance(texto, str): # Se verifica que la variable "texto" sea una cadena de texto.
...
def colocar_planta(fila:int, columna:int, planta_seleccionada:str, grilla:list, lista_plantas:list, cant_filas:int, cant_columnas:int, img_girasol:pygame.Surface, img_lanzaguisante:pygame.Surface, img_lanzaguisante_dispara:pygame.Surface, img_nuez:pygame.Surface, img_nuezmitad:pygame.Surface, img_nuezdañada:pygame.Surface, img_tralladora:pygame.Surface, img_tralladora_dispara:pygame.Surface) -> None:
    '''
    ...
    if 0 <= fila < cant_filas + 1 and 0 <= columna < cant_columnas: # Verifica que la fila y columna estén dentro de los límites de la grilla
        if grilla[fila][columna] == 0: # Si no hay planta en esa posición
            ...
            if planta_seleccionada == "girasol":
                nueva_planta = Girasol(fila, columna, img_girasol) # En caso de que la planta seleccionada sea un girasol, se crea un objeto de la clase Girasol
            elif planta_seleccionada == "lanzaguisante":
                nueva_planta = Lanzaguisantes(fila, columna, img_lanzaguisante, img_lanzaguisante_dispara) # En caso de que la planta seleccionada sea un lanzaguisante, se crea un objeto de la clase Lanzaguisantes
            elif planta_seleccionada == "nuez":
                nueva_planta = Nuez(fila, columna, img_nuez, img_nuezmitad, img_nuezdañada) # En caso de que la planta seleccionada sea una nuez, se crea un objeto de la clase Nuez
            elif planta_seleccionada == "lanzaguisanteTriple":
                nueva_planta = LanzaguisantesTriple(fila, columna, img_tralladora, img_tralladora_dispara) # En caso de que la planta seleccionada sea un lanzaguisante triple, se crea un objeto de la clase LanzaguisantesTriple

            if nueva_planta is not None: # Si se ha creado una planta nueva, se procede a agregarla a la lista de plantas y a la grilla
...
def generar_zombi(lista_zombis:list, zombis_disponibles:list, pesos:list, img_zombie_normal:pygame.Surface, img_zombie_cono:pygame.Surface, img_zombie_balde:pygame.Surface) -> None:
    '''
    ...
    lista_zombis.append(Zombie(tipo_zombi, imagen)) # Se crea un nuevo objeto Zombie con el tipo y la imagen seleccionada y se agrega a la lista de zombis


def generar_proyectil(lista_proyectiles: list, img_proyectil: pygame.Surface, x: int, y: int) -> None:
    '''
    ...
    lista_proyectiles.append(Proyectiles(x, y, img_proyectil)) # Crea un nuevo proyectil y lo agrega a la lista de proyectiles


def generar_soles(lista_soles: list, imagen: pygame.Surface, columna: int, fila: int) -> None:
    '''
    ...
    output:
        None
    '''
    nuevo_sol = Soles(columna, fila, imagen, "planta", 75, 75) # Crea un nuevo sol con la imagen, columna y fila especificadas
