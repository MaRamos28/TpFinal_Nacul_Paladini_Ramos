...
class Planta:
    ...
    def __init__(
        self, fila: int, columna: int, imagen: pygame.Surface, vida: int
    ) -> None:
        """
...
class LanzaguisantesTriple(Planta):
    ...
    def puede_disparar(self) -> bool:
        ...
        return (
            time.time() - self.ultimo_disparo >= self.cooldown
        ) and self.estado == "normal"
    ...
    def disparar(self, img_proyectil: pygame.Surface) -> list["Proyectiles"]:
        ...
        if self.estado == "disparando":
            if self.proyectiles_restantes > 0 and (
                tiempo_actual - self.ultimo_proyectil_disparado
                >= self.tiempo_entre_proyectiles
            ):
...
class Nuez(Planta):
    ...
    def __init__(
        ...
        fila: int,
        columna: int,
        imagen_sana: pygame.surface,
        imagen_mitad: pygame.surface,
        imagen_dañada: pygame.surface,
    ):
        """
        Inicializa una nuez en una posición específica de la grilla con diferentes estados de imagen.
        input:
            fila: fila en la que se va a iniciar la nuez
            columna: columna en la que se va a iniciar la nuez
            imagen_sana: imagen de la nuez en estado sano
            imagen_mitad: imagen de la nuez en estado medio dañado
            imagen_dañada: imagen de la nuez en estado dañado
    ...
    def actualizar_imagen(self):
        """
        Actualiza la imagen de la nuez según su vida actual.
        Si la vida es menor o igual a 15, usa la imagen dañada.
        Si la vida es menor o igual a 30, usa la imagen a mitad de daño.
        Si la vida es mayor a 30, usa la imagen sana.
        Esta función también actualiza el rectángulo de la nuez para reflejar su nueva imagen.
    ...
    def recibedaño(self):
        """
        Reduce la vida de la nuez en 1 punto y actualiza su imagen.
        output:
            True si la nuez ha sido destruida (vida <= 0), False en caso contrario.
...
class Proyectiles:
    """
    Clase que representa un proyectil disparado por una planta.
    """

    def __init__(self, x, y, imagen):
        """
        Inicializa un proyectil con su posición, imagen y velocidad.
        input:
    ...
    def mover(self):
        """
        Mueve el proyectil en la dirección horizontal.
        El proyectil se mueve a la derecha a una velocidad definida.
        Esta función actualiza la posición x del proyectil y su rectángulo asociado.
        """
    ...
    def dibujar(self, ventana, offset_y=0):
        ...
        input:
            ventana: superficie donde se dibuja el proyectil
            offset_y: desplazamiento vertical para el dibujo del proyectil
        """
    ...
    def devolver_coords(self):
        """
        Devuelve las coordenadas (x, y) del proyectil.
        output:
            (x, y) donde x es la coordenada horizontal y y es la coordenada vertical.
        """
        return (self.x, self.y)
